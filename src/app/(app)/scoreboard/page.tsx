...
export default async function ScoreboardPage({ searchParams }: { searchParams: { week?: string } }) {
  ...
  const rows = await Promise.all(
    reps.map(async (rep) => {
      const stats = await prisma.dailyStat.findMany({
        where: { userId: rep.id, date: { gte: weekStart, lt: weekEnd } },
      });
      const touches = await prisma.touch.findMany({
        where: { userId: rep.id, occurredAt: { gte: weekStart, lt: weekEnd } },
      });
      const dealsCreated = await prisma.deal.count({
        where: { ownerId: rep.id, createdAt: { gte: weekStart, lt: weekEnd } },
      });
      const dealsWon = await prisma.deal.count({
        where: { ownerId: rep.id, wonAt: { gte: weekStart, lt: weekEnd } },
      });

      const avgTargetPct = stats.length
        ? Math.round(stats.reduce((s, d) => s + (d.targetPct ?? 0), 0) / stats.length)
        : 0;
      ...
      const latestStreak = stats.at(-1)?.streakAfter ?? 0;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-[28px] tracking-tight m-0">Scoreboard</h1>
        <div className="flex gap-3 text-sm items-center">
          <Link href={`/scoreboard?week=${prevWeek.toISOString().slice(0, 10)}`} className="text-accent font-medium">
            ← Prior week
          </Link>
          <span className="text-muted">
            Week of {weekStart.toLocaleDateString()}
          </span>
          <Link href={`/scoreboard?week=${nextWeek.toISOString().slice(0, 10)}`} className="text-accent font-medium">
            Next week →
          </Link>
        </div>
      </div>
      ...
      <p className="text-xs text-muted mt-3">
        Review counts, average review score and top mistake tag land in Phase 3 once deal coaching reviews exist.
      </p>
    </div>
  );
}
